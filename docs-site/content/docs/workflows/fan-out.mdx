...
- **`first_success`** — succeeds if **at least one** branch succeeded.
...
visit cap returns `failed` rather than running again.
...
```bash
omp-squad add ~/code/myproject --workflow fan-out \
