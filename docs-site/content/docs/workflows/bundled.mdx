...
omp-squad ships five workflow graphs under `workflows/`. Point `--workflow` at one by name (or
at any `.fabro` path of your own) and `--task` becomes the run's goal.

```bash
omp-squad add ~/code/myproject --workflow plan-implement \
...
omp-squad's integration-layer resolver. When a feature branch won't land because main moved
