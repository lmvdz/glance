...
| `distill-plan` | A plan as IDEAS, not files: thesis you can disagree with, a metaphor from the plan's own domain, a visual per idea, honest state, drill-down provenance, identity per plan. Consumes `render-plan.ts --json` (same parser the HTML renderer uses) | DIRECTION.md's third human-contract clause (comprehend); exemplar built by hand in the memory-ledger lane, then decomposed into a six-rule contract |
