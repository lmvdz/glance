...
describe('staticRows', () => {
  test('blank query returns the nav rows + Org + Search tasks, in order', () => {
    ...
    expect(rows.map((r) => r.id)).toEqual([
      'nav-fleet', 'nav-tasks', 'nav-graph', 'nav-fog', 'nav-friction', 'nav-capabilities', 'nav-org', 'action-search-tasks',
    ]);
  });
  ...
  test('NAV_ROWS covers exactly the rail nav items + org, no dead views', () => {
    expect(NAV_ROWS.map((r) => r.view)).toEqual(['fleet', 'tasks', 'omp-graph', 'fog', 'friction', 'capabilities', 'org']);
  });
