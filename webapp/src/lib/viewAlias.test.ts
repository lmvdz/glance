...
describe('isAppView', () => {
  test('true for every current AppView key', () => {
    for (const v of ['fleet', 'tasks', 'omp-graph', 'fog', 'capabilities', 'org', 'intervene', 'review', 'plan-reality']) {
...
describe('coerceView — live keys pass through unchanged', () => {
  for (const v of ['fleet', 'tasks', 'omp-graph', 'fog', 'capabilities', 'org', 'intervene', 'review', 'plan-reality'] as const) {
