...
// Source: packages/core/src/Chat/useChatNewMessages.ts
// https://github.com/facebook/astryx/blob/deb5aa0/packages/core/src/Chat/useChatNewMessages.ts
//
// Adaptation from upstream: `.astryx-chat-message` class-selector lookup
// replaced with a `[data-chat-message]` attribute selector (see DESIGN.md
// and the sibling useChatStreamScroll.ts adaptation note).
 ...
 * @position Utility hook — used alongside useChatStreamScroll
 *
 * Observes a content element via ResizeObserver and tracks the last
 * `[data-chat-message]` element. When a new message appears (last element
 ...
 *
 * Returns a callback ref — pass it as the contentRef on the transcript
 * container. When the element mounts (even late), the observer attaches
 * automatically without needing state or version counters.
 */

import { useCallback, useEffect, useRef, useState } from "react";
import { observeResize, unobserveResize } from "../../lib/sharedResizeObserver";
...
export function useChatNewMessages({ isLocked, onResize }: UseChatNewMessagesOptions): UseChatNewMessagesReturn {
  ...
  const attach = useCallback((el: HTMLElement) => {
    observeResize(el, () => {
      ...
      const messages = el.querySelectorAll("[data-chat-message]");
