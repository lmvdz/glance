...
export const OmpGraphPanel: React.FC = () => {
      ...
      <div className="flex overflow-hidden rounded-md border border-ink-border">
        {RANGES.map((r) => (
          <button
            key={r}
            onClick={() => setDays(r)}
            className={`px-2.5 py-1 text-xs font-medium tabular-nums transition-colors ${days === r ? 'bg-orange-500 text-white' : 'bg-panel text-ink-text-muted hover:bg-ink-surface'}`}
            aria-pressed={days === r}
          >
            {r}d
          </button>
  ...
  return (
    ...
    <main className="flex h-full flex-1 flex-col overflow-hidden bg-panel">
      <div className="flex flex-shrink-0 flex-wrap items-center justify-between gap-x-4 gap-y-2 border-b border-ink-border px-4 py-2 border-ink-border">
        <div className="flex min-w-0 items-center gap-3">
          <Waypoints className="h-4 w-4 flex-shrink-0 text-orange-500" aria-hidden="true" />
          <h1 className="text-sm font-semibold text-ink-text">Graph</h1>
        ...
        {model && model.needsCount > 0 && (
          <button
            ...
            className="rounded-full border border-red-500/60 bg-red-500/10 px-2.5 py-1 text-caption font-semibold text-red-500 transition-colors hover:bg-red-500/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500"
          >
      ...
      <div className="flex min-h-0 flex-1">
        ...
        {!error && !model && empty && (
          <div className="flex flex-1 items-center justify-center p-4 text-center text-sm text-ink-text-muted">{empty}</div>
        )}
        {!error && !model && !empty && <div className="flex flex-1 items-center justify-center text-sm text-ink-text-subtle">Loading…</div>}
        {model && (
          <FleetPulseCanvas
            model={model}
