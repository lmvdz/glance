    ...
    <section className={`mb-6 overflow-hidden rounded-xl border ${t.border} ${t.softBg}`} aria-label="Agent status">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3">
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <span className="truncate text-sm font-semibold text-ink-text">{status.headline}</span>
        </div>
        {status.criteria && status.criteria.total > 0 && (
          <span className="flex shrink-0 items-center gap-2 text-xs text-ink-text-muted" title="Acceptance criteria met">
            <span className="h-1.5 w-20 overflow-hidden rounded-full bg-ink-border bg-ink-surface">
      ...
      {status.total > 0 && (
        <div className="flex flex-wrap gap-1.5 px-4 pb-3">
          {[...status.blockers.map((b) => b.agent), ...status.errored, ...status.working, ...status.idle, ...status.stopped]
            ...
            .map((a) => (
              <span key={a.id} className="inline-flex items-center gap-1.5 rounded-full border border-ink-border bg-white px-2 py-0.5 text-caption font-medium text-ink-text-label border-ink-border-2 bg-panel text-ink-text-label">
                <span className={`h-1.5 w-1.5 rounded-full ${STATUS_DOT[a.status] ?? 'bg-ink-text-subtle'}`} aria-hidden="true" />
                {a.name}
              </span>
            ))}
        </div>
      ...
      {status.blockers.length > 0 && (
        <div className="space-y-2 border-t border-amber-200/70 bg-white/60 px-4 py-3 dark:border-amber-900/40 bg-ink/40">
          {status.blockers.flatMap((b) =>
            b.requests.map((req) => (
              <div key={req.id} className="rounded-lg border border-amber-200 bg-amber-50/70 p-3 dark:border-amber-900/50 dark:bg-amber-950/20">
                ...
                {req.message && <p className="mt-1 whitespace-pre-wrap text-xs text-ink-text-label">{req.message}</p>}
                {req.options && req.options.length > 0 ? (
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {req.options.map((opt) => (
                      <button
                        ...
                        className="rounded-md border border-amber-300 bg-white px-2.5 py-1 text-xs font-medium text-amber-800 transition-colors hover:bg-amber-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 dark:border-amber-800 bg-panel dark:text-amber-300 dark:hover:bg-amber-950/40"
                      >
                        {opt}
                      </button>
                    ))}
                  ...
                  <div className="mt-2 flex gap-2">
                    <input
                      ...
                      autoFocus
                      className="flex-1 rounded-md border border-amber-300 bg-white px-2.5 py-1.5 text-xs text-ink-text-body placeholder:text-ink-text-subtle focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 dark:border-amber-800 bg-panel text-ink-text-body"
                    />
                    <button
                      type="button"
      ...
      {status.blockers.length === 0 && restartTargets.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 border-t border-ink-border/70 bg-white/60 px-4 py-3 border-ink-border/70 bg-ink/40">
          {restartTargets.map((a) => (
            <div key={a.id} className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1 rounded-md border border-emerald-300 bg-white px-2.5 py-1 text-xs font-semibold text-emerald-700 transition-colors hover:bg-emerald-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 dark:border-emerald-800 bg-panel dark:text-emerald-400 dark:hover:bg-emerald-950/40"
              >
      ...
      {status.primaryAction === 'implement' && hasPlan && (
        <div className="flex items-center justify-between gap-3 border-t border-ink-border/70 bg-white/60 px-4 py-3 border-ink-border/70 bg-ink/40">
          <span className="text-xs text-ink-text-label text-ink-text-subtle">This plan is ready — kick off an implementation agent.</span>
          <button
            type="button"
            onClick={onImplement}
            disabled={implementing}
            className="flex items-center gap-1.5 rounded-md bg-panel px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-ink-surface focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 disabled:cursor-not-allowed disabled:opacity-50 bg-ink-surface text-ink-text dark:hover:bg-ink-border"
          >
      ...
      {status.primaryAction === 'land' && status.vetoed.length > 0 && (
        <div className="space-y-1 border-t border-red-200/70 bg-white/60 px-4 py-3 text-xs text-red-700 dark:border-red-900/40 bg-ink/40 dark:text-red-400">
          <div className="flex items-center gap-2 font-semibold">
            <ShieldAlert className="h-3.5 w-3.5" aria-hidden="true" />
            Validator vetoed a green change — review before landing.
          </div>
          {status.vetoed.map((a) => (
            <div key={a.id} className="pl-6 text-ink-text-label text-ink-text-subtle">
              <span className="font-medium text-ink-text-body">{a.name}</span>
      ...
      {status.primaryAction === 'land' && status.vetoed.length === 0 && status.inconclusive.length > 0 && (
        <div className="space-y-1 border-t border-amber-200/70 bg-white/60 px-4 py-3 text-xs text-amber-700 dark:border-amber-900/40 bg-ink/40 dark:text-amber-400">
          <div className="flex items-center gap-2 font-semibold">
      ...
      {status.primaryAction === 'land' && status.vetoed.length === 0 && status.inconclusive.length === 0 && (
        <div className="flex items-center gap-2 border-t border-amber-200/70 bg-white/60 px-4 py-3 text-xs text-ink-text-label dark:border-amber-900/40 bg-ink/40 text-ink-text-subtle">
