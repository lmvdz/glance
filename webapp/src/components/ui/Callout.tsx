  ...
  return (
    <div className={`flex items-start gap-3 rounded-lg border ${t.border} ${t.softBg} px-4 py-3`} role={tone === 'critical' ? 'alert' : 'status'}>
      ...
      <div className="min-w-0 flex-1">
        <div className={`text-body font-semibold ${t.text}`}>{title}</div>
        {children != null && <div className="mt-0.5 text-caption text-ink-text-muted">{children}</div>}
      </div>
      {action && (
        <button
          onClick={action.onClick}
          className={`flex-shrink-0 rounded-md border ${t.border} bg-panel/50 px-2.5 py-1 text-caption font-medium ${t.text} transition-colors hover:bg-ink-surface-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-ember`}
        >
          {action.label}
        </button>
      )}
