    ...
    <div className={`flex items-start gap-3 rounded-lg border ${t.border} ${t.softBg} px-4 py-3`} role={tone === 'critical' ? 'alert' : 'status'}>
      ...
      {action && (
        <button
          ...
          className={`flex-shrink-0 rounded-md border ${t.border} bg-white/70 dark:bg-gray-900/50 px-2.5 py-1 text-xs font-medium ${t.text} transition-colors hover:bg-ink-surface-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500`}
        >
          {action.label}
        </button>
      )}
