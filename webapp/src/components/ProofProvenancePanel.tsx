...
export function ProofProvenancePanel({ task }: { task: Task }) {
  ...
  return (
    <section aria-label="Proof and provenance" className="rounded-xl border border-ink-border bg-white p-4 shadow-sm border-ink-border bg-ink">
      <div className="mb-3 flex items-center justify-between gap-3">
        <h2 className="text-sm font-semibold text-ink-text">Proof & provenance</h2>
        {readiness && <span className="rounded-full bg-ink-surface px-2 py-1 text-xs font-semibold text-ink-text-label bg-panel text-ink-text-body">{readiness.state.replace(/-/g, ' ')}</span>}
      </div>
      <dl className="space-y-3 text-sm">
        <div>
          <dt className="text-xs font-semibold uppercase tracking-wide text-ink-text0">Canon source</dt>
          <dd className="mt-1 text-ink-text">{data.source.label}</dd>
        </div>
        <div>
          <dt className="text-xs font-semibold uppercase tracking-wide text-ink-text0">Proof</dt>
          <dd className="mt-1 text-ink-text">{proofState} · fresh {proof?.fresh ?? 0} / failed {proof?.failed ?? 0} / stale {proof?.stale ?? 0} / none {proof?.none ?? 0} · ran {formatWhen(proof?.latestRanAt)} · {proof?.artifacts ?? 0} artifacts</dd>
        </div>
        <div>
          <dt className="text-xs font-semibold uppercase tracking-wide text-ink-text0">Next action</dt>
          <dd className="mt-1 text-ink-text">{readiness?.nextAction ?? 'Inspect candidate work.'}</dd>
        </div>
      </dl>
      {data.worktrees.length > 0 && (
        <div className="mt-4 space-y-2">
          <h3 className="text-xs font-semibold uppercase tracking-wide text-ink-text0">Candidate worktrees</h3>
          {data.worktrees.map((wt) => (
            <div key={`${wt.worktree}:${wt.branch ?? ''}`} className="rounded-lg bg-ink p-2 text-xs text-ink-text-label bg-panel text-ink-text-label">
              <div className="font-medium text-ink-text">{wt.agentName ?? wt.agentId ?? wt.branch ?? 'candidate'}</div>
      ...
      {data.candidates.length > 0 && (
        <div className="mt-4 space-y-2">
          <h3 className="text-xs font-semibold uppercase tracking-wide text-ink-text0">Plan revision candidates</h3>
