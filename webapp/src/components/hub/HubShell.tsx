    ...
    <header className="flex min-h-12 flex-shrink-0 items-center justify-between gap-4 border-b border-ink-border bg-panel px-4 py-2 text-ink-text">
      ...
      <div className="flex min-w-0 items-center gap-3" aria-label={label}>
        <div className="hidden min-w-0 items-center justify-end gap-1.5 sm:flex">
          {visible.map((user) => (
            <span key={user.id} className="max-w-32 truncate rounded-full border border-ink-border bg-panel px-2 py-1 text-[11px] text-ink-text-label" title={`${user.displayName} · ${user.socketCount} socket${user.socketCount === 1 ? '' : 's'}`}>
              {user.displayName}
              <span className="ml-1 text-ink-text-muted">×{user.socketCount}</span>
            </span>
          ))}
          {overflow > 0 ? <span className="rounded-full border border-ink-border bg-panel px-2 py-1 text-[11px] text-ink-text-muted">+{overflow}</span> : null}
        </div>
        ...
        <div className="flex items-center gap-1.5 text-xs text-ink-text-muted">
        ...
        {route.kind === 'workbench' ? renderWorkbench(route) : (
          <>
                    ...
                    <ol className="divide-y divide-ink-border">
                      {searchResults.map((result) => (
                        <li key={result.entry.id}>
                          <a href={hubHref(result.entry.channelId, result.entry.id)} onClick={() => setAnchorEntryId(result.entry.id)} className="block px-3 py-2 text-left hover:bg-panel focus-visible:ring-2 focus-visible:ring-ember focus-visible:ring-inset">
            ...
            <div className="border-t border-ink-border bg-ink">
              ...
              {replyTarget ? (
                <div className="flex items-center justify-between gap-3 border-b border-ink-border px-4 py-2 text-xs text-ink-text-muted">
                  <div className="min-w-0">
