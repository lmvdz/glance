...
test('an explicit variant overrides the status default variant', () => {
  ...
  expect(solid).toContain('border-ember');
  expect(solid).not.toContain('bg-ember text-ink');
});
...
test('human tone remains neutral rather than becoming a second accent', () => {
  // Mapped status…
  expect(renderToStaticMarkup(<StatusChip status="human" />)).toContain('bg-ink-surface');
  ...
  expect(named).toContain('border-ink-border-2'); // unknown labels default to outline
});

test('success tone renders emerald, distinct from ember', () => {
