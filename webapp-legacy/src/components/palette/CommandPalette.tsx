...
import { CORE_VIEWS, projectHash } from "@/lib/routes";
...
export function CommandPalette({ open, onOpenChange, squad, onView, onSelectAgent, onSelectFeature, onToggleTheme }: PaletteProps) {
  ...
  const entries = useMemo<Entry[]>(() => {
    const out: Entry[] = [];
    for (const v of CORE_VIEWS) out.push({ id: "view:" + v, label: v[0].toUpperCase() + v.slice(1), hint: "core view", run: () => onView(v) });
    for (const p of groupProjects(squad.features, squad.agents)) out.push({ id: "project:" + p.repo, label: p.name, hint: `${p.featureCount} missions · project`, run: () => { location.hash = projectHash(p.repo); } });
    for (const a of squad.agents) out.push({ id: "agent:" + a.id, label: a.name, hint: "agent · " + a.status, run: () => onSelectAgent(a.id) });
    for (const f of squad.features) out.push({ id: "feat:" + f.id, label: f.title, hint: "mission · " + f.stage, run: () => onSelectFeature(f.id) });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="top-[18%] max-w-xl translate-y-0 gap-0 overflow-hidden p-0">
        <input
          ...
          placeholder="Jump to a core view, project, agent, or mission…"
