...
export function appendTranscriptEntry(arr: TranscriptEntry[], entry: TranscriptEntry): TranscriptEntry[] {
  const match = entry.id ? arr.findIndex((item) => item.id === entry.id) : -1;
  if (match >= 0) {
    const next = [...arr];
    next[match] = entry;
    return next;
  }
  if (arr.some((item) => !entry.id && item.ts === entry.ts && item.kind === entry.kind && item.text === entry.text)) return arr;
  return arr.length >= TRANSCRIPT_CAP ? [...arr.slice(arr.length - TRANSCRIPT_CAP + 1), entry] : [...arr, entry];
}
        ...
        switch (ev.type) {
          ...
          case "transcript": {
            ...
            setTranscripts((prev) => {
              const m = new Map(prev);
              const arr = m.get(id) ?? [];
              m.set(id, appendTranscriptEntry(arr, entry));
              return m;
            });
            break;
          }
