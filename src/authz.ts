...
export function restActionTier(method: string, pathname: string): Role {
	...
	// Concern 09 (browser-audio-transport): this is a WS upgrade, so `method` is always GET by the
	// handshake's own nature — but granting the channel IS the mutating act (a holder can push mic PCM
	// into a real session, the same "driving the call" action `steer`/`resolveDecision`/mute already
	// gate at operator tier as POST routes). The coarse GET=viewer default below would be wrong here
	// specifically because the GET is the mutation. (This tier map is consulted only if a caller routes
	// the audio upgrade through it; server.ts's own upgrade block enforces this same floor directly,
	// since a WS handshake never reaches the REST dispatch this function's OTHER callers gate.)
	if (/^\/api\/channels\/[^/]+\/voice-call\/audio$/.test(pathname)) return "operator";
