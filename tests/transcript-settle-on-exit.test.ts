...
import type { PersistedAgent, RpcSessionState, SquadEvent, TranscriptEntry } from "../src/types.ts";
...
class DiesMidStreamDriver extends EventEmitter implements AgentDriver {
	...
	prompt(): Promise<void> {
		...
		return new Promise(() => {}); // the process dies before agent_end / tool_execution_end
	}
