 ...
 *  `clearVoicePushArmed` have something to act on) but drive the push lane itself via synthetic
...
test("a voice-armed working→idle transition drives a completion push through the server, and disarms the manager's latch", async () => {
	...
	// clearVoicePushArmed disarm needs a resident record to act on.
	...
	expect((mgr as unknown as InternalHost).agents.get(dto.id)?.options.voicePushArmed).toBe(true);
	...
	// covered by tests/voice-push-arm.test.ts) — seed, then the armed working→idle transition.
	mgr.emit("event", { type: "roster", agents: [agent("working", { id: dto.id, name: "voiced" })] });
	mgr.emit("event", { type: "agent", agent: agent("idle", { id: dto.id, name: "voiced", voicePushArmed: true }) });

	await sent; // resolves when the push is dispatched — no polling
	expect(calls).toEqual(["https://push.example.com/device"]);
	await waitFor(() => (mgr as unknown as InternalHost).agents.get(dto.id)?.options.voicePushArmed === false);
	expect((mgr as unknown as InternalHost).agents.get(dto.id)?.options.voicePushArmed).toBe(false);
});
...
test("an escalation and a completion push for the SAME agent within 3s of each other both send (separate debounce keys)", async () => {
	...
	mgr.emit("event", { type: "agent", agent: agent("input", { pending: [{ id: "p", source: "ui", kind: "select", title: "approve?", createdAt: 0 }] }) });
	mgr.emit("event", { type: "agent", agent: agent("idle", { voicePushArmed: true }) });
