...
test("no verified second provider lane: a cap on ANY provider still freezes the whole real-manager tick", async () => {
	// The pre-grok, pre-claude-code, pre-codex world: hold grok, claude-code, AND codex unverified so the
	// fleet has no vendor-pinned lane at all (b84904e flipped claude-code to verified:true off its own
	// live ACP smoke, and ticket #336 flipped codex likewise, 2026-08-04 — either alone now suffices to
	// make this fixture single-lane, so all three must be held down or this premise-pinning test would
	// silently stop covering the world it claims to). The ladder deps are wired but inert, and the legacy
	// no-arg top-of-tick check freezes everything on any cap.
	await withHarnessOverride("grok", { verified: false }, async () => {
		await withHarnessOverride("claude-code", { verified: false }, async () => {
			await withHarnessOverride("codex", { verified: false }, async () => {
...
test("claude-code ALONE (grok, codex held unverified) makes the lane REAL: a mismatched cap no longer freezes the default-harness tick", async () => {
	// Isolates claude-code as the sole second-lane contributor — grok AND codex are EXPLICITLY held
	// unverified via override, so if this still goes two-lane, it's b84904e's claude-code flip doing it,
	// not grok's or codex's pre-existing ones. claude-code is verified:true and pinned to `anthropic`,
	// which differs from the default `omp` harness's `unknown` baseline, so hasSecondVerifiedProviderLane()
	// is true through the REAL wired closure and dispatchProviderFor's per-unit gating goes live: an
	// "openai" cap must not touch units that resolve to the default (anthropic) bucket. This is the
	// freeze-isolation half.
	await withHarnessOverride("grok", { verified: false }, async () => {
		await withHarnessOverride("codex", { verified: false }, async () => {
			const { mgr, stop } = await makeWiredManager();
			const internals = mgr as unknown as ManagerInternals;
...
test("claude-code ALONE (grok, codex held unverified) does NOT open a hole: a cap on the ACTUAL resolved provider still pauses the tick", async () => {
...
test("grok makes the lane REAL out of the box: a mismatched cap no longer freezes the default-harness tick", async () => {
	// No override — this is the shipped registry. grok is verified:true and pinned to `xai` (codex, also
	// verified:true as of ticket #336, pins `openai` — either alone already makes this multi-lane), so
