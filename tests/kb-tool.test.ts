...
test("registerHostTools advertises squad_kb_search + squad_message to an omp agent", async () => {
	const { mgr, repo } = await mgrWithDecision();
	...
	expect(registered!.map((t) => t.name).sort()).toEqual(["squad_kb_search", "squad_message"]);
