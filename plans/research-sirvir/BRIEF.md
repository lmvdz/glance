...
### ⚠️ Post-research correction (adversarial /plan overturned this)
The strategist's "70% built, just reconnect the dead wire" claim below was **verified false against live `~/.glance` state** during the `/plan` design phase (2 red teams). The model-outcome ledger is **empirically empty** (recording never fired after 372 receipts), the router is wired **only to the interactive human spawn box** (the fleet's `autoRoute`/`routeIntake` path selects workflow/verify/thinking, not model — it never runs `shiftedModel`), and the proposed cost formula **fails closed**. The real gap is upstream: land-outcome recording doesn't work, and the fleet has no outcome-driven model selection at all. See `DESIGN.md` for the full arbitrated finding and the pivoted plan. Keep the strategist text below as the record of the pre-verification hypothesis.

### Bottom line (pre-verification — see correction above)
