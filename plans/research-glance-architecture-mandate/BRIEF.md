...
## 13. 2026-07-17 — Fourth review: Graphiti positioning (ADOPTED)

Question: does Graphiti (getzep/graphiti — temporal knowledge graphs for agents: episodes, entities, temporal relationship facts, custom ontologies, group_id namespacing, hybrid BM25+embedding+graph retrieval) change the plan? Verdict, adopted verbatim into ADR.md: **Graphiti fits as glance's optional temporal context/read layer — not the canonical Repository State Engine, not in the trusted landing path.** "Glance determines what is true. Graphiti helps agents find and reason over what glance knows."

Load-bearing boundaries (now in ADR.md + SCHEMA-V0.md's projection-compatibility section):
- Canonical plane stays glance-owned append-only records — content-addressed, exact-state-addressed, auditable, usable without Graphiti; a versioned async projector materializes them; the projection is disposable and rebuildable; Graphiti failure degrades context retrieval, never blocks a land.
- **Two ingestion paths, mandatory**: deterministic facts → direct CRUD writes with glance-assigned stable IDs (bypassing `add_episode`'s LLM extraction/resolution); interpretive sources (transcripts, papers, hypotheses) → extraction path, marked inferred, promoted only through glance evidence.
- **Auto-invalidation hazard**: Graphiti's contradiction resolution would let an unlanded candidate's facts invalidate accepted main's facts — the four KnowledgeSemantics axes project as explicit attributes, and query views (current/historical/proposed/counterfactual/transitions) are glance-defined recipes. (Supported by the "ghost memory" study arXiv:2607.01935 — explicit current/historical/transition labels improved Graphiti retrieval.)
- **Git-DAG validity**: RepositoryState entities + explicit transition edges (B→M, B→C, C→LANDED_AS→R, R→lineage), never commit timestamps mapped into valid_at fields; Graphiti's time fields carry observation/decision/attempt times only.
- Namespacing sketch: `repo:<id>:canonical | attempts | inferred`, `org:<id>:research | deliberation`.
- Rollout: Phases 0–2 design-compatibility only (already satisfied by SCHEMA-V0); Phase 3 optional read-model spike (historical attempts, exact IDs, rebuild-from-zero, query views, vs-JSONL comparison); Phase 4 `squad_kb_search` backend behind the existing membrane (fencing, scoping, budgets stay in front — never direct agent access); Phase 5 research/deliberation graphs (its strongest fit; Pydantic ontologies for Paper/Claim/Blocker/MAY_ADDRESS...).
- Deployment reality: Python sidecar + graph storage (Neo4j/FalkorDB/embedded FalkorDB Lite) on a TS-first local daemon ⇒ optional, async, rebuildable, never land-required.

Net effect on the plan: none to Phases 0–2 — the review is evidence the canonical substrate has a credible downstream consumer, and its requirements were already satisfied by the third review's schema. Documentation-only adoption: ADR Graphiti section, SCHEMA-V0 projection-compatibility contract, overview out-of-scope trigger.
