...
**Post-P1/P2 additions (same branch):** premium transcript rendering via `streamdown` (MIT, lazy + Shiki); a craft polish pass (a11y board buttons, loading skeletons, reduced-motion on slide-overs, consistent empty states); and **P3 surfaces** — a Network view (federation/presence/leases/Plane issues) + web push (Notify + `public/sw.js`). Push needs a `/sw.js` serve-seam line in `src/server.ts` for the flagged daemon path (works in dev/preview). Gate stays green: root `bun test` 492/0, webapp 14/0, build clean.
