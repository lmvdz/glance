...
- Gotcha (hit in C01): **linuxbrew's pkg-config shadows /usr/bin/pkg-config** on this host and searches only brew paths, so Tauri's system libs look missing even when installed. Every cargo invocation needs `PKG_CONFIG_PATH=/usr/lib/x86_64-linux-gnu/pkgconfig:/usr/share/pkgconfig`. Deps: upstream CI's four + `libdbus-1-dev` (WSL2 only).
