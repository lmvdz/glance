...
**Live-verification DONE (2026-07-10)**: probed the shipped `mintVoiceToken` against the real OpenAI GA `client_secrets` endpoint with a live key → returns a real `ek_` token (1h expiry, webrtc, pinnedAtMint). This caught a **ship-blocker**: the session body was the beta `/sessions` shape (flat `voice`/`turn_detection`/`input_audio_transcription`) which GA rejects 400 ("Missing required parameter: 'session.type'" / "Unknown parameter: 'session.voice'"). Fixed to the GA shape — `type:"realtime"` + audio nested under `session.audio.input`/`.output` — verified accepted live (commit adds this). The lane would have failed on its first real mint without this.
